...
class SupertrendGenerator:
    ...
    #------------------------------------------------------------------------------
    def trade_allowed(self, df, bbw_len=20, er_len=10, bbw_thr=0.04, er_thr=0.30):
        """
        Calculates Bollinger Band Width (BBW), Kaufman Efficiency Ratio (ER), & 
        Determines if the market is tradeable based on thresholds.
        
        Returns a boolean Series: 
            True  -> Trade allowed (non-sideways)
            False -> Sideways / avoid
        """
        close = df['close']
        ...
        upper = mid + 2 * std  # Using default std_mult=2
        lower = mid - 2 * std
        bbw = (upper - lower) / mid
        ...
        # Dynamic Thresholds (rolling means)
        bbw_thr = bbw.rolling(bbw_len).mean()  # Adaptive BBW threshold
        er_thr = er.rolling(er_len).mean()     # Adaptive ER threshold (or use a fixed value like 0.25 if preferred)
        
        # Tradeable Flag (using dynamic thresholds)
        bbw_dynamic = bbw > bbw_thr
        bbw_expanding = bbw > bbw.shift(1)
        tradeable = bbw_dynamic & bbw_expanding & (er > er_thr)
        return tradeable, bbw, er, bbw_expanding
    ...
    def generate_signals(self, lookback_bars: int = 10) -> List[Dict]:
        ...
        for i in range(100, len(self.data)):  # Start after EMAs stabilize
            ...
            bbw = self.data['bbw'].iloc[i]
            er = self.data['er'].iloc[i]
            bbw_expanding = self.data['bbw_expanding'].iloc[i]
            ...
            #Signal Types
            p1_breakout_long = self.data['p1_breakout_long'].iloc[i]
            p2_buy_pullback_long = self.data['p2_buy_pullback_long'].iloc[i]
            p3_trend_continuation_long = self.data['p3_trend_continuation_long'].iloc[i]
            p4_momentum_burst_long = self.data['p4_momentum_burst_long'].iloc[i]
            p1_breakout_short = self.data['p1_breakout_short'].iloc[i]
            p2_buy_pullback_short = self.data['p2_buy_pullback_short'].iloc[i]
            p3_trend_continuation_short = self.data['p3_trend_continuation_short'].iloc[i]
            p4_momentum_burst_short = self.data['p4_momentum_burst_short'].iloc[i]
            ...
            #modify the signal dictionary creation:
            if signal_type == 'LONG':
                p1_breakout_long = (
                    (closing_price > self.data['EMA200'].iloc[i]) &
                    (bbw < 0.04) &
                    (er < 0.35) &
                    (bbw_expanding) &
                    (closing_price > self.data['High'].rolling(20).max().shift(1).iloc[i]) &
                    (self.data['Volume'].iloc[i] > 1.2 * self.data['avg_volume_20'].iloc[i])
                )
                p2_buy_pullback_long = (
                    (closing_price > self.data['EMA200'].iloc[i]) &
                    (self.data['ema20'].iloc[i] > self.data['ema50'].iloc[i]) &
                    (self.data['EMA3_Close'].iloc[i] < self.data['EMA8_Close'].iloc[i]) &
                    (closing_price >= self.data['ema50'].iloc[i] * 0.995) &
                    (rsi > 40) & (rsi < 60)
                )
                p3_trend_continuation_long = (
                    (closing_price > self.data['EMA200'].iloc[i]) &
                    (self.data['ema20'].iloc[i] > self.data['ema50'].iloc[i]) & (self.data['ema50'].iloc[i] > self.data['EMA200'].iloc[i]) &
                    (self.data['EMA3_Close'].iloc[i] > self.data['EMA8_Close'].iloc[i]) &
                    (trend == 'up_trend')
                )
                p4_momentum_burst_long = (
                    (closing_price > self.data['EMA200'].iloc[i]) &
                    (self.data['EMA3_Close'].iloc[i] > self.data['EMA8_Close'].iloc[i]) &
                    (self.data['atr'].iloc[i] > self.data['atr'].rolling(10).mean().iloc[i] * 1.3) &
                    (self.data['Volume'].iloc[i] > 1.5 * self.data['avg_volume_20'].iloc[i]) &
                    (rsi > 60)
                )
                # Set on DataFrame
                self.data.at[self.data.index[i], 'p1_breakout_long'] = p1_breakout_long
                self.data.at[self.data.index[i], 'p2_buy_pullback_long'] = p2_buy_pullback_long
                self.data.at[self.data.index[i], 'p3_trend_continuation_long'] = p3_trend_continuation_long
                self.data.at[self.data.index[i], 'p4_momentum_burst_long'] = p4_momentum_burst_long
                # For SHORT, set to None
                self.data.at[self.data.index[i], 'p1_breakout_short'] = None
                self.data.at[self.data.index[i], 'p2_buy_pullback_short'] = None
                self.data.at[self.data.index[i], 'p3_trend_continuation_short'] = None
                self.data.at[self.data.index[i], 'p4_momentum_burst_short'] = None
                ...
                p1_breakout_short = (
                    ...
                    (er < 0.35) &
                    (bbw_expanding) &
                    (closing_price < self.data['Low'].rolling(20).min().shift(1).iloc[i]) &
                    (self.data['Volume'].iloc[i] > 1.2 * self.data['avg_volume_20'].iloc[i])
                )
                p2_buy_pullback_short = (
                    (closing_price < self.data['EMA200'].iloc[i]) &
                    (self.data['ema20'].iloc[i] < self.data['ema50'].iloc[i]) &
                    (self.data['EMA3_Close'].iloc[i] > self.data['EMA8_Close'].iloc[i]) &
                    (closing_price <= self.data['ema50'].iloc[i] * 1.005) &
                    (rsi > 40) & (rsi < 60)
                )
                p3_trend_continuation_short = (
                    (closing_price < self.data['EMA200'].iloc[i]) &
                    (self.data['ema20'].iloc[i] < self.data['ema50'].iloc[i]) & (self.data['ema50'].iloc[i] < self.data['EMA200'].iloc[i]) &
                    (self.data['EMA3_Close'].iloc[i] < self.data['EMA8_Close'].iloc[i]) &
                    (trend == 'down_trend')
                )
                p4_momentum_burst_short = (
                    (closing_price < self.data['EMA200'].iloc[i]) &
                    (self.data['EMA3_Close'].iloc[i] < self.data['EMA8_Close'].iloc[i]) &
                    (self.data['atr'].iloc[i] > self.data['atr'].rolling(10).mean().iloc[i] * 1.3) &
                    (self.data['Volume'].iloc[i] > 1.5 * self.data['avg_volume_20'].iloc[i]) &
                ...
                # Set on DataFrame
                self.data.at[self.data.index[i], 'p1_breakout_short'] = p1_breakout_short
                self.data.at[self.data.index[i], 'p2_buy_pullback_short'] = p2_buy_pullback_short
                self.data.at[self.data.index[i], 'p3_trend_continuation_short'] = p3_trend_continuation_short
                self.data.at[self.data.index[i], 'p4_momentum_burst_short'] = p4_momentum_burst_short
                # For LONG, set to None
                self.data.at[self.data.index[i], 'p1_breakout_long'] = None
                self.data.at[self.data.index[i], 'p2_buy_pullback_long'] = None
                self.data.at[self.data.index[i], 'p3_trend_continuation_long'] = None
                self.data.at[self.data.index[i], 'p4_momentum_burst_long'] = None
            else:
                # No signal, set all to None
                self.data.at[self.data.index[i], 'p1_breakout_long'] = None
                self.data.at[self.data.index[i], 'p2_buy_pullback_long'] = None
                self.data.at[self.data.index[i], 'p3_trend_continuation_long'] = None
                self.data.at[self.data.index[i], 'p4_momentum_burst_long'] = None
            ...
            signal = {
                ...
                'P1L_BREAKOUT': p1_breakout_long,
                'P2L_BUY_PULLBACK': p2_buy_pullback_long,
                'P3L_TREND_CONTINUATION': p3_trend_continuation_long,
                'P4L_MOMENTUM_BURST': p4_momentum_burst_long,
                'P1S_BREAKOUT': p1_breakout_short,
                'P2S_BUY_PULLBACK': p2_buy_pullback_short,
                'P3S_TREND_CONTINUATION': p3_trend_continuation_short,
                'P4S_MOMENTUM_BURST': p4_momentum_burst_short,
    ...
    def run(self, lookback_bars: int = 10) -> List[Dict]:
        ...
        """
        self.data['p1_breakout_long'] = None
        self.data['p2_buy_pullback_long'] = None
        self.data['p3_trend_continuation_long'] = None
        self.data['p4_momentum_burst_long'] = None
        self.data['p1_breakout_short'] = None
        self.data['p2_buy_pullback_short'] = None
        self.data['p3_trend_continuation_short'] = None
        self.data['p4_momentum_burst_short'] = None
        ...
        tradeable, bbw, er, bbw_expanding = self.trade_allowed(temp_df)
        self.data['tradeable'] = tradeable
        self.data['bbw'] = bbw
        self.data['er'] = er    
        self.data['bbw_expanding'] = bbw_expanding
