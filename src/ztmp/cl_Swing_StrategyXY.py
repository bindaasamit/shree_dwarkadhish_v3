...
class SwingTradingStrategyV3:
    ...
    def __init__(self, data: pd.DataFrame, symbol: str = "Unknown", fast_ema_period: int = 3, slow_ema_period: int = 8, show: bool = False):
        """
    ...
    def calculate_emas(self, show: bool = False) -> None:
        if self.show  == True:
            print("""Calculate all required EMAs for the strategy.""")
        # Fast EMAs (self.fast_ema_period-period) - Orange Channel
        self.data[self.fast_high_col] = self.data['High'].ewm(span=self.fast_ema_period, adjust=False).mean()
        self.data[self.fast_low_col] = self.data['Low'].ewm(span=self.fast_ema_period, adjust=False).mean()
        self.data[self.fast_close_col] = self.data['Close'].ewm(span=self.fast_ema_period, adjust=False).mean()
        
        # Medium EMAs (self.slow_ema_period-period) - White Channel
        self.data[self.slow_high_col] = self.data['High'].ewm(span=self.slow_ema_period, adjust=False).mean()
        self.data[self.slow_low_col] = self.data['Low'].ewm(span=self.slow_ema_period, adjust=False).mean()
        self.data[self.slow_close_col] = self.data['Close'].ewm(span=self.slow_ema_period, adjust=False).mean()
    ...
    def calculate_supertrend(self, atr_period: int = 10, factor: float = 3.0, show: bool = False) -> None:
        """
        ...
        """
        if self.show  == True:
            print("""Calculate SuperTrend indicator.""")
        high = self.data['High']
        low = self.data['Low']
    ...
    def calculate_rsi(self, period: int = 14, show: bool = False) -> None:
        """Calculate RSI and its momentum characteristics."""
        if self.show  == True:
            print("""Calculate RSI and its momentum characteristics.""")
    ...
    def calculate_volume_metrics(self, show: bool = False) -> None:
    ...
    def detect_market_regime(self, show: bool = False) -> None:
        """
        ...
        """
        if self.show  == True:
            print("""Detect market regime using Bollinger Band Width and Efficiency Ratio.""")
        close = self.data['Close']
    ...
    def calculate_candle_metrics(self, show: bool = False) -> None:
        if self.show == True:
            print("""Calculate candle-specific metrics for entry safety.""")
        high = self.data['High']
        low = self.data['Low']
        close = self.data['Close']
    ...
    def is_move_exhausted(self, i: int, direction: str = 'LONG', show: bool = False) -> bool:
        """
    ...
    def has_recent_trap(self, i: int, direction: str = 'LONG', lookback: int = 5, show: bool = False) -> bool:
        """
    ...
    def calculate_smart_stop(self, i: int, direction: str = 'LONG', show: bool = False) -> float:
        """
    ...
    def check_liquidity(self, i: int, min_value: float = 5_000_000, show: bool = False) -> bool:
        """
    ...
    def check_pattern_failure(self, entry_signal: Dict, current_idx: int, show: bool = False) -> tuple[bool, str]:
        """
        ...
        if signal_type == 'LONG':
            if pattern == 'MOMENTUM_BURST':
                ...
                if not price_above_ema200:
                    return True, 'Price no longer > EMA200'
                if not ema3_above_ema8:
                    return True, 'EMA3 no longer > EMA8'
                if not ema200_slope_positive:
                    return True, 'EMA200 slope not positive'
                ...
                #adx = ta.adx(self.data['High'], self.data['Low'], self.data['Close'], length=14).iloc[current_idx]
                ...
                if not close_above_range:
                    return True, 'Close no longer above range'
                if not htf_trend_up:
                    return True, 'HTF trend not up'
                if not no_lower_high:
                    return True, 'Lower high formed'
                if not regime_allows_expansion:
                    return True, 'Regime no longer allows expansion'
        ...
        # SHORT positions
            ...
            if pattern == 'MOMENTUM_CRASH':
                ...
                if not close_below_support:
                    return True, 'Close no longer below support'
                ...
                if not price_below_dynamic_resistance:
                    return True, 'Price no longer below dynamic resistance'
                if not pullback_is_corrective:
                    return True, 'Pullback not corrective'
                if not time_decay_ok:
                    return True, 'Time decay not ok'
                ...
                if not no_regime_shift:
                    return True, 'Regime shift occurred'
    ...
    def calculate_dynamic_stop(self, signal_type, pattern, entry_price, bars_held, data, entry_ATR, show=False):
        if self.show  == True:
            print("""calculate_dynamic_stop.""")
    ...
    def check_stop_loss(self, signal_type, entry_price, stop_loss, bars_held, data, show=False):
        if self.show  == True:
            print("""check_stop_loss.""")
        close = data["Close"]
        high  = data["High"]
        low   = data["Low"]
        ema20 = data["EMA20"]
    ...
    def detect_pattern_d_momentum_burst(self, i: int, show: bool = False) -> Tuple[bool, Dict]:
        """
        ...
        - atr_condition: ATR(14) > 1.2 × SMA(ATR, 20) for at least 2 of last 3 bars
        - volume_above_ma: Volume > 1.2 × SMA(Volume, 20)
        - volume_above_prev: Volume > Volume[1]
        - rsi_in_range: RSI(14) ≥ 60 AND RSI(14) ≤ 78
        - rsi_slope_positive: RSI(14) > RSI(14)[1]
        - cross_recent: Bars_since(EMA3 crossed above EMA8) ≤ 25
        
        Entry: If structure_valid and quality_score >= 2
        """
        if self.show  == True:
            print("""detect_pattern_d_momentum_burst.""")
        if i < 100:
            return False, {}
        ...
        structure_valid = price_above_ema200 and ema3_above_ema8 and ema200_slope_positive
        ...
        atr_condition = sum(1 for k in range(i-2, i+1) if self.data.iloc[k]['ATR'] > 1.2 * avg_atr_20) >= 2
        ...
        # Calculate quality score as sum of conditions
        quality_score = sum([
            atr_condition,
            volume_above_ma,
            volume_above_prev,
    ...
    def detect_pattern_a_breakout(self, i: int, show: bool = False) -> Tuple[bool, Dict]:
        """
        ...
        Quality Score: Sum of optional conditions (0-5)
        ...
        """
        if self.show  == True:
            print("""detect_pattern_a_breakout.""")
        ...
        #adx = ta.adx(self.data['High'], self.data['Low'], self.data['Close'], length=14).iloc[i]
    ...
    def detect_pattern_c_continuation(self, i: int, show: bool = False) -> Tuple[bool, Dict]:
        """
        ...
        """
        if self.show  == True:
            print("""detect_pattern_c_continuation.""")
    ...
    def detect_pattern_b_pullback(self, i: int, show: bool = False) -> Tuple[bool, Dict]:
        """
        ...
        """
        if self.show  == True:
            print("""detect_pattern_b_pullback.""")
    ...
    def detect_pattern_d_momentum_crash_short(self, i: int, show: bool = False) -> Tuple[bool, Dict]:
        """
        ...
        if self.show  == True:
            print("""detect_pattern_d_momentum_crash_short.""")
        ...
        # Quality score
        quality_score = sum([
            downside_space,  # pullback_at_support
            volume_expansion,  # volume_contraction (but expansion for crash)
            rsi_crashing,  # rsi_supportive
            breakdown_fresh,  # clean_entry_trigger
            atr_expansion  # atr_contraction_then_expansion
        ])
    ...
    def detect_pattern_a_breakdown_short(self, i: int, show: bool = False) -> Tuple[bool, Dict]:
        """
        ...
        """
        if self.show  == True:
            print("""detect_pattern_a_breakdown_short.""")
        ...
        # Structure valid (assuming close_below_support instead of ema200, etc.)
        structure_valid = (
    ...
    def detect_pattern_c_continuation_short(self, i: int, show: bool = False) -> Tuple[bool, Dict]:
        """
        ...
        """
        if self.show  == True:
            print("""detect_pattern_c_continuation_short.""")
    ...
    def detect_pattern_b_rally_short(self, i: int, show: bool = False) -> Tuple[bool, Dict]:
        """
        ...
        - rsi_rollover_or_divergence: RSI rollover or divergence
        ...
        """
        if self.show  == True:
            print("""detect_pattern_b_rally_short.""")
        ...
        rs = avg_gain / avg_loss
        rsi = 100 - (100 / (1 + rs))
        rsi_rollover = (rsi.iloc[i] >= 60) and (rsi.diff().iloc[i] < 0)
        rsi_divergence = (
    ...
    def generate_entry_signals(self, show: bool = False) -> List[Dict]:
        """
        ...
        for i in range(100, len(self.data)):
            ...
            if signal_type and position_size > 0:
                ...
                signal = {
                    ...
                    'rsi': data['RSI'],
                    'rvol': data['RVol'],
                    'quality_score': pattern_details.get('quality_score', None),
    ...
    def generate_exit_signals(self, entry_signal: Dict, start_idx: int, show: bool = False) -> Dict:
        """
    ...
    def run(self, show: bool = False) -> Tuple[List[Dict], pd.DataFrame]:
        """
        ...
        self.calculate_emas(show=show)
        self.calculate_supertrend(show=show)
        self.calculate_rsi(show=show)
        self.calculate_volume_metrics(show=show)
        self.detect_market_regime(show=show)
        self.calculate_candle_metrics(show=show)
        
        #Generate signals
        entry_signals = self.generate_entry_signals(show=show)
        
        # Add exit signals to each entry
        for signal in entry_signals:
            exit_info = self.generate_exit_signals(signal, signal['index'], show=show)
