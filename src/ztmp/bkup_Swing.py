...
class SwingTradingStrategyV2:
    ...
    def detect_pattern_d_momentum_burst(self, i: int) -> Tuple[bool, Dict]:
        """
        Pattern D: Momentum Burst
        
        Catches explosive moves with strong volume and volatility expansion.
        High risk, high reward pattern for aggressive entries.
        Best when markets transition from ranging to trending.
        
        Conditions:
        - Price > EMA200 (bullish regime)
        - EMA3 > EMA8 (momentum direction)
        - ATR > 1.3x average ATR (volatility spike)
        - Volume > 1.5x average (institutional participation)
        - RSI > 60 (strong momentum, not overbought yet)
        - SuperTrend GREEN (trend confirmation)
        ...
        # Strong but not exhausted RSI
        rsi_strong = data['RSI'] > 60 and data['RSI'] < 80
    ...
    def generate_exit_signals(self, entry_signal: Dict, start_idx: int) -> Dict:
        ...
        for i in range(start_idx + 1, len(self.data)):
            ...
            if self.check_pattern_failure(entry_signal, i):
                exit_price = data['Close']
                pnl = ((exit_price - entry_price) / entry_price * 100
                    if signal_type == 'LONG'
                    else (entry_price - exit_price) / entry_price * 100)
                ...
                return {
                    ...
                    'pnl_pct': pnl,
                    'bars_held': bars_held,
                    'exit_details': f'{pattern} failed early'
                }
